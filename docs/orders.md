...
The Next.js BFF exposes fixed `/api/products` and `/api/orders` routes. Browser JavaScript still never receives API tokens. The RabbitMQ Worker that publishes `OrderPaid` and creates Notifications remains the next unfinished backend component.
